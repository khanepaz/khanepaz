...
<label class="field"><span>دسته</span><select id="ca">${cats.map(x=>`<option value="${x.slug}" ${x.slug===r.category?"selected":""}>${esc(x.name)}</option>`).join("")}</select></label>
