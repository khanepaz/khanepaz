...
(() => {
  ...
  document.addEventListener('click', e => {
    ...
    if (t.dataset.action === 'share') {
      ...
      navigator.clipboard?.writeText(url).then(() => alert('پیوند کپی شد')).catch(() => prompt('پیوند:', url));
      return;
    }
  ...
  $('#cmForm')?.addEventListener('submit', e => {
    ...
    if (!name || !text) return;
    const c = {
      name, text, rating: rate,
      recipeId: $('#cmRecipe').value || null,
      date: new Date().toISOString().slice(0, 10),
      local: true
    };
    S.comments.unshift(c);
    try {
      const key = 'kp-comments';
  ...
  async function boot() {
    try {
      S = await window.DataAPI.loadAll();
      // local comments
      try {
      ...
      console.error(e);
      $('#loader').innerHTML = '<p>خطا در بارگذاری داده</p>';
    }
  }
  boot();
})();
