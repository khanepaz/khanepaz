...
function edCar(i){const n=i<0;const s=n?{recipeId:data.recipes?.[0]?.id||"",badge:"",title:"",text:""}:structuredClone(data.site.carousel[i]);
...
$("#save").onclick=()=>{const o={recipeId:$("#rid").value,badge:$("#bd").value.trim(),title:$("#ti").value.trim(),text:$("#tx").value.trim()};if(n)data.site.carousel.push(o);else data.site.carousel[i]=o;mark("site");$("#modal").hidden=true;render();toast("OK")}}

function edT(i){const n=i<0;const t=n?{id:"",title:"",level:"مبتدی",duration:10,icon:"📖",color:"#f2b632",summary:"",steps:[""],note:""}:structuredClone(data.tutorials[i]);
